...
static void ulongToString( unsigned long long int value, 
                          char *&current )
{
   *--current = 0;
   do
...
std::string valueToString( Value::Int value )
{
   ...
   if ( isNegative )
      value = -value;
   ulongToString( Value::UInt(value), current );
   if ( isNegative )
      *--current = '-';
...
std::string valueToString( Value::Long value )
{
   char buffer[32];
   char *current = buffer + sizeof(buffer);
   bool isNegative = value < 0;
   if ( isNegative )
      value = -value;
   ulongToString( Value::ULong(value), current );
   if ( isNegative )
      *--current = '-';
   assert( current >= buffer );
   return current;
...
std::string valueToString( Value::ULong value )
{
   char buffer[32];
   char *current = buffer + sizeof(buffer);
   ulongToString( value, current );
